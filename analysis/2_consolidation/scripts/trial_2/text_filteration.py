...
class EmbeddingModel:
    ...
    def generate_embeddings_for_corpus(self, corpus):
        ...
        normalized_embeddings = np.array(
            [emb / np.linalg.norm(emb) for emb in embeddings], dtype="float32"
        )
...
class FaissDB:
    ...
    def search(self, query_embedding, k=2):
        ...
        return distances.reshape(-1), [
            self.embedding_documents[index] for index in indices.reshape(-1)
        ]
...
def calculate_embedding_similarity(source_embedding, target_embeddings):
    ...
    similarity_scores = cosine_similarity(
        source_embedding.reshape(1, -1), target_embeddings
    )
    return similarity_scores
...
class CCF:
    ...
    def __init__(self):
        ...
        self.faiss_db = FaissDB(
            self.embed_model.model.get_sentence_embedding_dimension()
        )

    def filter_text(
        self,
        reference_text: str,
        victim_corpus: list[str],
        ...
        start_doc_similarity = calculate_embedding_similarity(
            reference_text_embedding, start_doc["embedding"].reshape(1, -1)
        )
        ...
        if start_doc["id"] > 0:
            ...
            self.faiss_db.add_embeddings(
                victim_corpus[:index_boundary], embeddings[:index_boundary]
            )
            ...
            while iteration_count < max_iterations:
                ...
                for assumed_target_link in range(current_doc["id"] - 1, -1, -1):
                    max_depth = (
                        assumed_target_link
                        if max_depth > assumed_target_link
                        else max_depth
                    )
                    if not document_list.get(assumed_target_link):
                        ...
                        for batch_no in range(max_batches):
                            ...
                            batch_results = self.faiss_db.search(
                                current_doc_embedding,
                                k=search_batch_size * (batch_no + 1),
                            )
                            batch_results = (
                                batch_results[0][batch_jump_size:],
                                batch_results[1][batch_jump_size:],
                            )
                            for dist, result_doc in zip(
                    ...
                    similarity_score = calculate_embedding_similarity(
                        current_doc_embedding, result_doc["embedding"].reshape(1, -1)
                    )
                    is_assumend_target_link_fit = (
                        similarity_score > similarity_threshold
                    )
                    if is_assumend_target_link_fit:
                        filtered_corpus.insert(
                            0, document_list[assumed_target_link]["doc"]
                        )
        ...
        if start_doc["id"] < len(victim_corpus) - 1:
            ...
            while iteration_count < max_iterations:
                ...
                current_doc_embedding = current_doc["embedding"]
                document_list = {}
                for assumed_target_link in range(
                    current_doc["id"] + 1, forward_corpus_size
                ):
                    max_depth = (
                        assumed_target_link
                        if max_depth < assumed_target_link
                    ...
                    if is_assumend_target_link_fit:
                        filtered_corpus.append(
                            document_list[assumed_target_link]["doc"]
                        )
                        current_doc = result_doc
                        break
